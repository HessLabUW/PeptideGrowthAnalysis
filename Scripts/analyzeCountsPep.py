...
parser.add_argument('-s', '--split', dest='split_mark',
                        help='Delimiter for element name. Default is __',
                type=str, default='__')
...
with open(file_out + '.csv', 'w') as out_open:
    ...
    elements_header=[]
    for a in range (0, max_elements):
      elements_header.append("Element #"+str(a+1))

    # Writes a header
    out_csv.writerow(['#GeneID', 'Symbol',
                        'Element #','Median Effect',
                        'casTLE Effect', 'casTLE Score', 'casTLE p-value',
			'Minimum Effect Estimate', 'Maximum Effect Estimate']+ elements_header)
	...
	# Converts IDs as neccessary
        ...
        # Writes to file
        out_csv.writerow([geneID, name, num, med,
...
with open(file_out + '_record.txt', 'w') as rec_open:
    ...
    rec_csv.writerow(['Draw number', int(np.median(list(map(len, gene_rhos.values()))))])
